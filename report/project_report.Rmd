...
* Functions for Proportional Odds Model
    + pom.est: return the estimates of intercepts and slopes of a proportional odds model. It calls the following functions:
...
To fit the proportional odds model, we use the `optimx` function with BFGS as the method, and supply the likelihood and gradient functions given above. We also explored fitting a penalized proportional odds model to our data with an elastic net penalty by coordinate descent with the `ordinalNet` package.
