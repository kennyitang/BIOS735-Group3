...
4. Temperature: numeric, degrees Fahrenheit    
5. Humidity: numeric, percentage  
6. Pressure: numeric air pressure, in inches  
7. Visibility: numeric, in miles  
8. Wind Direction --- aggregate CALM vs NOT CALM?  (not using right now)
9. Wind Speed: numeric, in mph 
10. Weather Condition -- aggregate? (53 factors)  (not using right now)
  
**Points of Interest**  

11. Crossing: TRUE/FALSE, presence of a crossing nearby  
12. Traffic_Signal: TRUE/FALSE, presence of a traffic signal nearby  

# Methods 
First, we apply a proportional odds model with the severity of an accident as the outcome, and start time ,<---insert list of candidate variables--->, as predictors. Based on paramter estimates of this model <---potentially with penalization--->, we identify variables that are most associated with severity. In parallel, starting with the same set of variables, we use random forest to find the predictors and build a model that best yields the best classification.  
