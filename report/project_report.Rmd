...
The final dataset used contains 119,279 traffic accidents from North Carolina between the years 2016 and 2019. The original dataset had 49 total attributes, including attributes related to traffic, location, weather, points of interest, and period of day. For our purposes, we removed most location attributes (?? not done yet). We also chose to remove Wind Chill and Precipitation since they had around 60% missingess. Additionally, most of the points of interest variables had very low variability across observations, so we chose only to keep Crossing and Traffic Signal.

In terms of traffic incidents, rows were removed that were missing all weather variables. Additionally, the data were split up into training data (pre-2019) and testing data (2019) as mentioned previously. The final training dataset has 75,887 observations and the testing has 43,392.

```{r}
data_filtered <- read.csv("../data/NC_Accidents_filtered.csv")
data_training <- read.csv("../data/NC_Accidents_trn.csv")
data_testing <- read.csv("../data/NC_Accidents_tst.csv")
```
...
Additionally, the variable had four categories (1-4) but only 24 observations had severity of 1, so we combined severities 1 and 2 and readjusted the scale.  

It should be noted that the dataset sources are rather vague on the definition and derivation of this outcome variable. Length of delay caused by the accident is mentioned, and we assume that scope, such as how many lane closures there are, may also be taken into account when determining the severity. 
...
1. Source: categorical, API that reported accident (MapQuest, Bing, MapQuest-Bing) -- remove?   
2. Start Time -- start time of the accident. keep the hour and minute of the day? aggregate?, keep month?  
3. End Time   -- end time of accident remove? 
4. Start Lat -- remove?  
5. Start Lng -- remove?  
6. Description: in Natural Language, description of the accident  -- remove for now?  
7. Street: character, street name -- remove?  
8. Side: categorical, side of street (Left/Right)  
9. City -- remove for now? or aggregate rural/urban? (YC: then we need some way to distinguish between rural/urban)  
10. County -- remove for now? or aggregate rural/urban?  
11. Zipcode -- remove   
12. Airport Code -- remove (86 levels)    
13. Sunrise_Sunset: categorical (Day/Night)  
  
**Weather**  

14. Weather Timestamp -- remove  
15. Temperature: numeric, degrees Fahrenheit    
16. Humidity: numeric, percentage  
17. Pressure: numeric air pressure, in inches  
18. Visibility: numeric, in miles  
19. Wind Direction --- aggregate CALM vs NOT CALM?  
20. Wind Speed: numeric, in mph 
21. Weather Condition -- aggregate? (53 factors)  
  
**Points of Interest**  

22. Crossing: TRUE/FALSE, presence of a crossing nearby  
23. Traffic_Signal: TRUE/FALSE, presence of a traffic signal nearby  
...
, where $\beta = (\beta_1,...,\beta_p)^T$, $x_i = (x_{i1},..., x_{ip})^T$, k=1, 2, and 3,.

Equivalently, if we define $$\phi(t) = \frac{1}{1+exp(-t)}$$
, then $$P(Y_i\leq k) = \frac{1}{1 + exp(x_i^T\beta-\alpha_k)} = \phi(\alpha_k-x_i^T\beta)$$

.The log likelihood function for n accidents is therefore given by 
...
, where $\alpha = (\alpha_1,\alpha_2, \alpha_3)^T.$

The gradient is $(\frac{\partial l(\alpha, \beta)}{\partial \beta_1}, ..., \frac{\partial l(\alpha, \beta)}{\partial \beta_p}, \frac{\partial l(\alpha, \beta)}{\partial \alpha_1},..., \frac{\partial l(\alpha, \beta)}{\partial \alpha_3})^T$, and the individual elements are given below. 
...
$$ \frac{\partial l(\alpha, \beta)}{\partial \alpha_1} = \sum_{i=1}^n \left( I(Y_i=1)\frac{exp(x_i^T\beta-\alpha_1)}{1+exp(x_i^T\beta-\alpha_1)}-I(Y_i=2)\frac{\frac{exp(x_i^T\beta-\alpha_1)}{[1+exp(x_i^T\beta-\alpha_1)]^2}}{\phi(\alpha_2-x_i^T\beta)-\phi(\alpha_1-x_i^T\beta)} \right)=\sum_{i=1}^n\left( I(Y_i=1)[1-\phi(\alpha_1-x_i^T\beta)] - I(Y_i=2)\frac{\phi(\alpha_1-x_i^T\beta)[1-\phi(\alpha_1-x_i^T\beta)]}{\phi(\alpha_2-x_i^T\beta)-\phi(\alpha_1-x_i^T\beta)} \right)$$

$$ \frac{\partial l(\alpha, \beta)}{\partial \alpha_2} = \sum_{i=1}^n \left( I(Y_i=2)\frac{\frac{exp(x_i^T\beta-\alpha_2)}{[1+exp(x_i^T\beta-\alpha_2)]^2}}{\phi(\alpha_2-x_i^T\beta)-\phi(\alpha_1-x_i^T\beta)}-I(Y_i=3)\frac{\frac{exp(x_i^T\beta-\alpha_2)}{[1+exp(x_i^T\beta-\alpha_2)]^2}}{\phi(\alpha_3-x_i^T\beta)-\phi(\alpha_2-x_i^T\beta)} \right)=\sum_{i=1}^n\left( I(Y_i=2)\frac{\phi(\alpha_2-x_i^T\beta)[1-\phi(\alpha_2-x_i^T\beta)]}{\phi(\alpha_2-x_i^T\beta)-\phi(\alpha_1-x_i^T\beta)} - I(Y_i=3)\frac{\phi(\alpha_2-x_i^T\beta)[1-\phi(\alpha_2-x_i^T\beta)]}{\phi(\alpha_3-x_i^T\beta)-\phi(\alpha_2-x_i^T\beta)} \right)$$

$$ \frac{\partial l(\alpha, \beta)}{\partial \alpha_3} = \sum_{i=1}^n \left( I(Y_i=3)\frac{\frac{exp(x_i^T\beta-\alpha_3)}{[1+exp(x_i^T\beta-\alpha_3)]^2}}{\phi(\alpha_3-x_i^T\beta)-\phi(\alpha_2-x_i^T\beta)}-I(Y_i=4)\frac{\frac{exp(x_i^T\beta-\alpha_3)}{[1+exp(x_i^T\beta-\alpha_3)]^2}}{1-\phi(\alpha_3-x_i^T\beta)} \right)=\sum_{i=1}^n\left( I(Y_i=3)\frac{\phi(\alpha_3-x_i^T\beta)[1-\phi(\alpha_3-x_i^T\beta)]}{\phi(\alpha_3-x_i^T\beta)-\phi(\alpha_2-x_i^T\beta)} - I(Y_i=4)\frac{\phi(\alpha_3-x_i^T\beta)[1-\phi(\alpha_3-x_i^T\beta)]}{1-\phi(\alpha_3-x_i^T\beta)} \right)$$
