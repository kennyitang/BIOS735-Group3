...
# Data
The original dataset had 49 total attributes, including attributes related to traffic, location, weather, points of interest, and period of day.  Pre-processing the data set was necessary to allow us to focus on the variables and observations of real relevance to our aim.    

For our purposes, we removed most location attributes. After aggregating the County variable into urban and rural factor levels, it was apparent that about 95% of the accident data was derived from urban areas. Therefore this information was not useful for prediction. We also chose to remove Wind Chill and Precipitation since they had around 60% missingess. Additionally, most of the points of interest variables had very low variability across observations, so we chose to only keep Crossing and Traffic Signal.The analysis dataset contains 119,279 traffic accidents from North Carolina between the years 2016 and 2019.
...
The variables selected for analysis, and some accompany exploratory plots are as follows: 
...
11. Weather Condition -- aggregate? (53 factors)  (not using right now)
...
12. Crossing: TRUE/FALSE, presence of a crossing nearby for pedestrians, cyclists, etc. 
13. Traffic_Signal: TRUE/FALSE, presence of a traffic signal nearby on an intersection 
...
One way to evaluate the performance of our methods is to test the accuracy of the models on the test set. To do this, the fraction of the correctly classified observations is divided by the total number of observations in the test set. 
...
We can also compare the computational time it takes to build each model. This is important when working with large datasets. 
...
ordinalForest took about 1.5 hours. 
